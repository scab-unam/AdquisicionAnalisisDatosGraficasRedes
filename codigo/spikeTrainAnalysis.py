...
def xtractTrains(dataDir,prefix,suff=".mat",varName="st"):
    """ 
    Example:
    tt=xtractTrains(dataDir,prefix,suff=".mat")
    """
    trains=list()
    data = dict()
    ff=getFileList(dataDir,prefix,suff)
    tt=readMatlabFileList(dataDir,ff,"st")
    return tt

def readMatlabSpikeTrains(dataDir,files,varName):
    nFiles=len(files)
    data= list()
    for n in sc.arange(nFiles):
        fName = files[n]
        st= spikeTrain()
        st.spikeTimes=sc.squeeze(io.loadmat(dataDir + fName)[varName][0][0])
        st.calcISI()
        st.calcIFR()
        st.fName=fName
        data.append(st)
...
def xtractSpikeTrains(dataDir,prefix,suff=".mat",varName="st"):
    """ 
    Example:
    tt=xtractTrains(dataDir,prefix,suff=".mat")
    """
    ...
    data = dict()
    ff=getFileList(dataDir,prefix,suff)
    tt=readMatlabSpikeTrains(dataDir,ff,"st")
    return tt
...
class spikeTrain:
    st=sc.array([0,0])
    nBursts=0
    def calcISI(self, order=1):
        self.nSpikes = len(self.spikeTimes)
        isi = sc.zeros(self.nSpikes)
        isi[order:] = self.spikeTimes[order:] - self.spikeTimes[:-order]
        return isi
        
    def calcIFR(self, order=1):
        self.nSpikes = len(self.spikeTimes)
        ifr = sc.zeros(self.nSpikes)
        ifr[order:] = 1/(self.spikeTimes[order:] - self.spikeTimes[:-order])
        return ifr

    def graphISIHist(self,isi,binEdges,ax):
        ax.hist(isi,binEdges)

    def graphIFRHist(self,ifr,binEdges,ax):
        ax.hist(ifr,binEdges)

    def burstIFR(self,thresh):
        bTimes=list(); bStarts=list(); bEnds=list()
        bI= sc.where(self.ifr>=thresh)[0]
        bInds=list()
        if len(bI)>0:   
            i0=bI[0]
            if i0>0:
                bInds.append(i0-1)
                bInds.append(i0)
                bStarts.append(self.spikeTimes[i0-1])
        
            for n in sc.arange(len(bI)):
                if bI[n]-i0<2:
                    bInds.append(bI[n])
                    bTimes.append(self.spikeTimes[bI[n]])               
                else:
                    aa=bI[n]-1
                    bInds.append(aa)
                    bInds.append(bI[n])
                    bStarts.append(self.spikeTimes[aa])
                    bEnds.append(self.spikeTimes[bI[n-1]])
                    bTimes.append(self.spikeTimes[aa])
                    bTimes.append(self.spikeTimes[bI[n]])
                i0=bI[n]
            bEnds.append(self.spikeTimes[bI[-1]])
            bTimes.append(self.spikeTimes[bI[-1]])
            self.bStarts=sc.unique(sc.array(bStarts))
            self.bEnds=sc.unique(sc.array(bEnds))
            self.bTimes=sc.unique(sc.array(bTimes))
            self.bInds=sc.unique(sc.array(bInds))
            print(self.fName)
            print("% bursts", 100*len(self.bInds)/sc.float64(len(self.spikeTimes)))
            self.nBursts= len(self.bStarts)
            self.bDurations=self.bEnds-self.bStarts
            self.bIntervals= sc.zeros(self.nBursts)
            self.bDuty = sc.zeros(self.nBursts)
            self.bQuiesc = sc.zeros(self.nBursts)
            self.bIntervals[:-1]=self.bStarts[1:]-self.bStarts[:-1]
            self.bDuty[:-1]= (self.bEnds[:-1]-self.bStarts[:-1])/self.bIntervals[:-1]
            self.bQuiesc[:-1]= self.bStarts[1:]-self.bEnds[:-1]
            # Count the number of spikes per burst
            self.nSpikesPerBurst = list()
            for n in sc.arange(self.nBursts):
                a=self.bStarts[n]
                b=self.bEnds[n]
                nsb=len(sc.where( (self.spikeTimes>=a)&(self.spikeTimes<=b))[0])
                self.nSpikesPerBurst.append(nsb)
            # Stats on bursting
            if 0:
                self.bDurationsMin=self.bDurations.min()
                self.bDurationsMean=self.bDurations.mean()
                self.bDurationsMax=self.bDurations.max()
                self.bIntervalsMin=self.bIntervals.min()
                self.bIntervalsMean=self.bIntervals.mean()
                self.bIntervalsMax=self.bIntervals.max()
                self.bDutyMin=self.bDuty.min()
                self.bDutyMean=self.bDuty.mean()
                self.bDutyMax=self.bDuty.max()
                self.bQuiescMin=self.bQuies.min()
                self.bQuiescMean=self.bQuies.mean()
                self.bQuiescMax=self.bQuies.max()
        else:
            self.bInds=[]
            self.bTimes=[]; self.bStarts=[]; self.bEnds=[]
            self.nBursts=0; self.bDurations=[]; self.bIntervals=[]
            self.bDuty=[]; self.bQuiesc=[]
        return self.bInds
...
def ifrBurstParameters(train,alpha=0.5):
    bp=dict()
    nSpikes=len(train)
    ...
    upThresh= calcThresholdsFromCDF(cdfInverse, (alpha,))
    dnThresh= calcThresholdsFromCDF(cdfInverse, (1-alpha,))
...
def preAnalyseTrains(dataDir,prefix,suff=".mat",varName="st", aBinSize=0.05, maxTime=1.0,thresh=0.95):
    trains=list()
    data = dict()
    ff=sta.getFileList(dataDir,prefix,suff)
    tt=sta.readMatlabArray(dataDir,ff,"st")
    nTrains=len(tt)
    dd=list()
    for nn in sc.arange(nTrains):
        train=tt[nn]
        dd.append(sta.preAnalyseTrain(train, aBinSize, maxTime, thresh))
    return dd
